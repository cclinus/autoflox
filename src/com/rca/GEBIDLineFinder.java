...
public class GEBIDLineFinder {
	...
	private List<FunctionTrace> relevantSeq; //List of function traces in the relevant sequence
	...
	private FunctionTrace gebidLineFt; //The function trace corresponding to the GEBID line
	
	private boolean calledFinder; //true if findGEBIDLine has already been called
	
	/**
	 * The GEBIDLineFinder constructor
	 * 
	 * @param nullVar The variable that became null
	 * @param seq A list of function traces representing the relevant sequence
	 */
	public GEBIDLineFinder(String nullVar, List<FunctionTrace> seq) {
		...
		ListIterator<FunctionTrace> seqIt = relevantSeq.listIterator(relevantSeq.size());
	 ...
	 * 
	 * @param folder The JS source folder
	 */
	...
	/**
	 * Finds the GEBID line and, once found, sets the value of gebidLineFt to the corresponding FunctionTrace
	 * <p>
	...
	public void findGEBIDLine() {
		...
		//Set up iterator for the relevant sequence and determine the name of the current function
		ListIterator<FunctionTrace> seqIterator = relevantSeq.listIterator(relevantSeq.size()); //start at end of the list
		...
		while (seqIterator.hasPrevious()) {
			...
			if (!(currType.equals("ASYNC_CALL") || currType.equals("ASYNC"))) {
				...
				//Check the first line (i.e., the erroneous line) as this line itself might be the one containing the GEBID line
			...
			if (foundAsync) {
				...
				if (!currType.equals("ASYNC_CALL")) {
					System.out.println("Error: ASYNC not preceded by an ASYNC_CALL");
					System.exit(-1);
				}
				else {
					foundAsync = false;
			...
			if (funcStackIsEmpty()) {
				...
				if (!isJSFunctionCall(lineTokens, last_func)) {
					System.out.println("Error: Primary function changed, but no corresponding function call");
					System.exit(-1);
				}
				else {
			...
			if (!currFunction.equals(funcStack.peek())) {
				...
				if (isFuncCallAfterPeek(relevantSeq, currIndex, currFunction, callingFunction)) {
					//First, verify that the current expression is a return expression
					if (!isReturnExpr(lineTokens)) {
						System.out.println("Error: Function from which return value is expected does not end with return expression");
						System.exit(-1);
					}
					else {
						...
						//Check if the return expression is one of the GEBID functions
						if (isNullSource(retExpr)) {
							...
							System.out.println("Error: Return value is not a GEBID, a function call, or a variable name");
							System.exit(-1);
						}
					}
				}
					...
					if (wasReturned) {
						...
						if (!indexToChangeModified) {
							System.out.println("Error: Invalid indexToChange value about to be used");
							System.exit(-1);
						}
			...
			if (isNullVarAssn(lineTokens)) {
				...
				if (isNullSource(expr)) {
					...
					System.out.println("Error: Expression is neither a GEBID nor a variable name");
					System.exit(-1);
				}
			}
			else {
	...
	/**
	 * Returns a function trace representing the line determined to contain the GEBID call
	 * 
	 ...
	 * 
	 * @param ft The function trace to set gebidLineFt to
	 */
		...
		while (tokenIt.hasNext()) {
			...
			switch(currentState) {
				...
				case St_CONT_LOOKING:
					...
					break;
				default:
					System.out.println("Error: Incorrect state in isJSFunctionCall");
					System.exit(-1);
					break;
			}
		}
	...
	private boolean isVarLocal(FunctionTrace ft, String varName) {
		if (ft == null || varName == null || varName == "") {
			System.out.println("Error: Empty variable or trace passed to isVarLocal");
			System.exit(-1);
		}
		...
		if (!varFound) {
			System.out.println("Error: variable name not found in function declaration");
			System.exit(-1);
		}
	 ...
	 */
	private String getArgument(List<String> tokenList, int paramNum, String funcName) {
		//Set up states
		final int
			St_START = 1,
			St_VAR_DECL = 2,
		...
		try {
			...
			if ((strLine = br.readLine()) != null) {
				...
				if (token_stream_it.hasNext()) {
					String firstToken = (String)token_stream_it.next();
					if (!firstToken.equals("FUNCTION")) {
						System.out.println("Error: First line is not a function declaration");
						System.exit(-1);
					}
				}
				else {
					System.out.println("Error: No tokens in function declaration");
					System.exit(-1);
				}
			}
			else {
	 ...
	 */
	private boolean isFuncCallAfterPeek(List<FunctionTrace> trace_list, int curr_index, String currFunction, String callingFunc) {
	 ...
	 */
	private boolean isNullVarEqualsFuncCallAssn(List<String> tokenList, String funcName) {
		final int
			St_START = 1,
			St_VAR_DECL = 2,
			St_LP_START = 3,
		...
		while (tokenIt.hasNext()) {
			...
			switch(currentState) {
				...
				default:
					System.out.println("Error: Incorrect state in isNullVarEqualsFuncCallAssn");
					System.exit(-1);
					break;
			}
		}
	 ...
	 * 
	 * @param tokenList The list of tokens
	 * @return true if the corresponding list of tokens corresponds to a function call; false otherwise
	 */
	...
	/**
	 * Determines if the current line is of the form <null var> = expression.
	 * If so, it returns the tokens representing the right hand side of this assignment.
	 * If not, returns an empty list
	 * 
	 * @param tokenList The list of tokens
	 * @return A list of tokens representing the expression on the right hand side if the provided list of
	 * tokens is of the form <null var> = expression; otherwise, returns an empty list of strings
	 */
	...
	private String getVariableName(List<String> tokenList) {
		if (!isVariable(tokenList)) {
			System.out.println("Error: Attempting to extract variable name from tokens not representing a variable");
			System.exit(-1);
		}
	...
	/**
	 * Checks if the list of tokens represents a call to document.getElementById() or $()
	 * 
	 * @param tokenList The list of tokens
	 * @return true if tokenList represents a call to document.getElementById() or $(); false otherwise
	 */
	 ...
	 * 
	 * @param tokenList The list of tokens
	 * @return true if tokenList represents a call to getAttribute; false otherwise
	 */
	 ...
	 * 
	 * @param tokenList The list of tokens
	 * @return true if tokenList represents a call to getComputedStyle; false otherwise
	 */
	private boolean isGCS(List<String> tokenList) {
		final int
			...
			St_GCS_FOUND = 7,
			St_DOCUMENT_FOUND = 8,
			St_FIRST_DOT_FOUND = 9,
			St_DEF_VIEW_NAME_FOUND = 10,
			St_DEF_VIEW_FOUND = 11;
	...
	/**
	 * Checks if the list of tokens represents an expression that could have started the propagation of null
	 * 
	 * @param tokenList The list of tokens
	 * @return true if the tokenList expression could have started null propagation; false otherwise
	 */
	...
	private void testGEBIDLineFinderMethods() {
		...
		Parser ps = new Parser(new CompilerEnvirons());
		TokenStream ts = ps.initForUnitTest(new StringReader("(x = hello(why,hello));"), "", 1, false);
