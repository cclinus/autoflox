 ...
 */
public class JSExecutionTracer implements PreStateCrawlingPlugin,
		PostCrawlingPlugin, PreCrawlingPlugin, OnNewStatePlugin,
		GeneratesOutput {
	...
	public void preStateCrawling(CrawlSession session,
		...
		String filename = getOutputFolder() + EXECUTIONTRACEDIRECTORY
				+ "jsexecutiontrace-";
		...
		try {
			...
			rcaGui.setOutputConsole("Reading execution trace");

			LOGGER.info("Parsing JavaScript execution trace");
			rcaGui.setOutputConsole("Parsing JavaScript execution trace");
			...
			rcaGui.setOutputConsole("Saved execution trace as " + filename);

			points = new JSONArray();

			// Update progress bar
			rcaGui.setProgressBar(95);
